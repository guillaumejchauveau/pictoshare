...
class FileSystemDocumentSource: DocumentSource {
    ...
    required init(with config: Configuration, uuid: UUID) {
        ...
        do {
            try monitoredFolder = FileManager.default.url(for: .documentDirectory,
                                                          in: .userDomainMask,
                                                          appropriateFor: nil,
                                                          create: true).appendingPathComponent("PTSFolder", isDirectory: true).path
            ...
            print("FATAL ERROR : Failed to launch the FSE")
        }
    ...
    func promptDocument(with config: Configuration) {
        openPanel.begin(completionHandler: {
            ...
            if (response == NSApplication.ModalResponse.OK) {
                self.importCallback?(TextDocument(content: "", documentName: "title"))
            }
        })
        openPanel.runModal()
    }
        ...
        if !event.path.contains(".DS_STORE") {
            ...
            if(event.flag) != nil {
                ...
                if( (event.flag?.contains(EonilFSEventsEventFlags.itemRenamed))! &&
                    ...
                    let fileName = String(event.path[event.path.index(after: firstIndex)..<lastIndex])

                    let fileData = FileManager.default.contents(atPath: event.path)
                    
                    // For now, we just assume that the file is a text file in UTF8
                    if(fileData != nil) {
                        let textDocument = TextDocument(content: String(decoding: fileData!, as: UTF8.self),
                        ...
                        do {
                            try pdfExporter.export(document: textDocument, with: [:])
                        } catch {
                            print("Erreur lors de l'exportation vers le PDF")
                        }

                        
                        //FileManager.default.createFile(
                        //    atPath: urlThatSaved.path,
                        //    contents: fileData)
                    }
                    else {
